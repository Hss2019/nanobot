...
## 2026-03-13 会话 31 — Brave 配置修复、失败重试与工具结果预览

**操作者**: Codex GPT-5 + 用户 Hss

### 完成事项

1. **修复 Brave Search API Key 被掩码值覆盖**
   - `nanobot/webui/static/index.html`
   - 设置页保存时，如果 Brave key 仍是 `***` 掩码，不再把它写回配置
   - 避免用户保存其他设置时把真实 Brave key 覆盖坏

2. **失败回复支持重试**
   - `nanobot/webui/static/index.html`
   - 仅对失败回复显示“重试”按钮
   - 重试会复用上一条用户输入与已上传图片路径
...
3. **工具调用结果支持展开预览**
   ...
   - `nanobot/webui/static/index.html`
   - 每次工具执行后发送一条结果预览
   - 对话区可展开查看工具输出，不再只能看到工具调用名

4. **自定义端点 `max_tokens` 限制兼容**
   - `nanobot/providers/custom_provider.py`
   - `nanobot/agent/loop.py`
   - 当自定义端点明确返回 `max_tokens` 上限时，自动按该上限重试一次
   - 并把错误解释成人话，提示用户接口限制和设置调整方向

---

## 2026-03-13 会话 32 — 收尾优化：模型预设、托盘菜单、README 收敛
...
1. **设置页支持模型预设管理**
   ...
   - `nanobot/webui/server.py`
   - `nanobot/webui/static/index.html`
   - 新增 `agents.model_presets` 与 `agents.active_model_preset_id`
   - 支持：
     - 保存当前模型配置为预设
     - 选择预设
     - 切换预设
     - 删除预设
   - 预设只保存模型运行参数，不复制 API Key / API Base

2. **工具输出并入同一条调用记录**
   - `nanobot/webui/static/index.html`
   - 工具结果不再单独生成一条“查看输出”卡片
   - 现在会直接挂到对应的工具调用记录下面
...
4. **README 收敛为对外说明**
   - `README.md`
   - 移除了不适合首页承载的内部部署 / 验证细节
   - 改为引用 `.docs/deploy.md` 与 `.docs/changelog.md`
...
1. **图片识别兼容性补全**
   - 当前 WebUI 上传与消息链路已通
   - 但 `custom` 端点下的视觉输入仍受目标端点多模态兼容性限制
   - 后续需要补 provider 级能力检测与适配
